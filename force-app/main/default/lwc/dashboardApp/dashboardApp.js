...
const URL =
  "https://services9.arcgis.com/N9p5hsImWXAccRNI/arcgis/rest/services/Z7biAeD8PAkqgmWhxG2A/FeatureServer/1/query?f=json&where=Confirmed%20%3E%200&outFields=Country_Region,Confirmed,Deaths,Recovered,Last_Update,Active&orderByFields=Confirmed%20desc";
let initialValue = {
  total_deaths: 0,
  total_confirmed: 0,
  total_active: 0,
  ...
  formatData(result) {
    ...
    let finalData = Object.keys(individualSum).map((data) => {
      ...
      let activeColumnClass =
        item.Recovered < item.Active ? "activeColumnClass" : "";
      let recoveredColumnClass =
        item.Recovered > item.Active ? "recoveredColumnClass" : "";
      let fatalityColumnClass =
        this.getFRate(item) > this.getFRate()
          ? "fatalityColumnClass-danger"
          : this.getFRate(item) < this.getFRate()
          ? "fatalityColumnClass-success"
          : "";
      let recoveryColumnClass =
        this.getRRate(item) > this.getRRate()
